...
export function Table({
    ...
    function resolveBackgroundColor(): string {
        if (isTooSmall || isFilteredOut) return colors.Neutral;
    ...
    const disabled = !isSelectable || isFilteredOut || isTooSmall;
    ...
    return (
        <button
            ...
            type="button"
            disabled={disabled}
            title={isTooSmall ? "Table is too small": undefined}
            onClick={() => onSelect?.(id)}
        ...
        >
            {windowLocation.left && capacity > 3 && (
                <div
                    className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-3/5 bg-black/40 rounded-r-full"/>
            )}

            {windowLocation.top && capacity > 3 && (
                <div
                    className="absolute top-0 left-1/2 -translate-x-1/2 h-1 w-3/5 bg-black/40 rounded-b-full"/>
            )}
            ...
            {isOccupied && !isFilteredOut && (
                <span
                    className="absolute -top-2 -right-2 rounded-full bg-slate-700 px-1.5 py-0.5 text-xs text-white">
					Occupied
				</span>
            )}
            ...
            {isBestTable && !isFilteredOut && (
                <span
                    className="absolute -bottom-2 -left-2 rounded-full bg-amber-400 px-1.5 py-0.5 text-xs text-slate-900">
					Best
				</span>
            )}
        </button>
